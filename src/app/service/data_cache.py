...
class DataCache:
    ...
    def load_from_sheet(self, sheet_id: str, sheet_name: str, run_indexes: List[int]) -> None:
        ...
        try:
            ...
            external_refs = self._collect_external_references(all_values, run_indexes, col_to_field)
            ...
            for idx in run_indexes:
                try:
                    if idx > len(all_values):
                        logger.warning(f"Row {idx} is out of range (sheet has {len(all_values)} rows)")
                        skipped_count += 1
                    ...
                    if cached_row:
                        ...
                        if loaded_count % 10 == 0:
                            logger.info(f"Processed {loaded_count}/{len(run_indexes)} rows")
                    else:
                        skipped_count += 1
                        skipped_rows.append(idx)
    ...
    def _collect_external_references(
        ...
        ref_configs = {
            ...
            'blacklist': ['U', 'V', 'W']   # IDSHEET_BLACKLIST, SHEET_BLACKLIST, CELL_BLACKLIST
        }
            ...
            for ref_type, cols in ref_configs.items():
                try:
                    ...
                    if all(col_idx < len(row_values) for col_idx in col_indices):
                        sheet_id = row_values[col_indices[0]].strip() if col_indices[0] < len(row_values) else ""
                        sheet_name = row_values[col_indices[1]].strip() if col_indices[1] < len(row_values) else ""
                        cell = row_values[col_indices[2]].strip() if col_indices[2] < len(row_values) else ""
                    ...
                    logger.debug(f"Error collecting external ref for row {idx}, type {ref_type}: {e}")
    ...
    def _parse_row_to_cached(
        ...
        try:
            ...
            def get_external_key(id_col: str, sheet_col: str, cell_col: str) -> Optional[str]:
            ...
            if include_kw:
                include_keywords_value = [kw.strip() for kw in include_kw.split(';') if kw.strip()]
            
            exclude_keywords_value = None
            exclude_kw = get_value('Z')
            if exclude_kw:
                exclude_keywords_value = [kw.strip() for kw in exclude_kw.split(';') if kw.strip()]
